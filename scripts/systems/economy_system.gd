...
func run(state: Dictionary) -> Dictionary:
	...
	if workshop_count > 0 and not workshop_paused:
		var convert_rate: float = (1.2 + workshop_count * 0.9) * convert_mult
		var convert: float = minf(float(resources["stone"]), convert_rate * delta)
		resources["stone"] = float(resources["stone"]) - convert
		resources["cobblestone"] = float(resources["cobblestone"]) + convert
